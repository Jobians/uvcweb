...
unsafe fn device_ids(h: *mut UsbHandle) -> Option<(u16, u16)> {
  ...
  Some((u16::from_le_bytes([buf[8], buf[9]]), u16::from_le_bytes([buf[10], buf[11]])))
}

impl Capture {
  /// Open the card. On failure returns the process exit code (same numbers as the C version).
  pub fn open(cfg: &Config) -> Result<Capture,
  i32> {
    unsafe {
      ...
      if r != 0 {
        say!("warning: libusb device-discovery option failed: {}", usb_err(r));
      }
      ...
      if cfg.width == 0 || cfg.height == 0 {
        ...
        match descriptors::default_mode(&modes) {
          Some((w, h, f)) => {
            ...
            mh = h as i32;
            mf = f as i32;
            say!("using the card's default mode: MJPEG {}x{} @ {} fps", mw, mh, mf);
          }
          None => {
      ...
      let e = uvc_get_stream_ctrl_format_size(devh, cap.ctrl.as_mut_ptr() as *mut c_void, UVC_FRAME_FORMAT_MJPEG, mw, mh, mf);
      if e < 0 {
        uvc_print_diag(devh, std::ptr::null_mut());
      ...
      let e = uvc_start_streaming(devh, cap.ctrl.as_mut_ptr() as *mut c_void, on_frame, std::ptr::null_mut(), 0);
      if e < 0 {
        say!("start_streaming failed: {} (try a lower resolution or fps)", uvc_err(e));
        uvc_close(devh);
        uvc_exit(ctx);
        return Err(4);
      }
  ...
  pub fn usb_handle(&self) -> *mut UsbHandle {
    unsafe {
      uvc_get_libusb_handle(self.devh)
    }
  }
  ...
  String> {
    ...
    unsafe {
      ...
      if e >= 0 {
        e = uvc_start_streaming(self.devh, self.ctrl.as_mut_ptr() as *mut c_void, on_frame, std::ptr::null_mut(), 0);
      }
      if e < 0 {
